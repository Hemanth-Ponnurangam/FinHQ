...
async function fetchHistory(ticker, range) {
  ...
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(t)}?range=${range}&interval=${intervalMap[range]||'1d'}`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const d = await res.json();
...
export function initWealth(ui) {
  ...
  function updateStatusBar(lastFetchTs) {
    ...
    // Show next Friday 3:30 IST
    const nextFriday = scheduleAutoFetch(() => refreshAllPrices());
