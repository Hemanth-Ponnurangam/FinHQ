...
export function initDebt(ui) {
  ...
  // Log EMI quick action in detail sheet
  document.getElementById('debtDetailLogEMIBtn')?.addEventListener('click', async () => {
    const debt = store.debts.find(d => d.id === currentDetailDebtId);
    if (!debt) return;
    const btn = document.getElementById('debtDetailLogEMIBtn');
    const now = new Date();
    ...
    try {
      ...
      if (btn) { btn.textContent = '✓ Logged'; btn.className = btn.className.replace('red', 'green'); }
    } catch (err) {
      console.error('Log EMI failed:', err);
      if (btn) { btn.textContent = 'Log EMI'; btn.disabled = false; }
    }
  });
