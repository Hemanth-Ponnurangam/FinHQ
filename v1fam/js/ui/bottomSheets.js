export function initUI() {
  ...
  const allSheets = [
    ...
    document.getElementById('kiteImportSheet'),
  ].filter(Boolean);
  ...
  document.getElementById('executeConfirmBtn')?.addEventListener('click', (e) => {
    const btn = e.currentTarget;
    // Guard: ignore if already executing (rapid double-tap / click) or no pending callback.
    if (btn.disabled || !currentConfirmCallback) return;

    // Disable immediately — the sheet slides out in 300 ms, but a fast second tap
    // can still fire the handler during that window and double-execute the action.
    btn.disabled = true;

    // Snapshot and clear the callback before invoking so a re-entrant call
    // (theoretically possible if the callback synchronously opens another confirm)
    // doesn't see stale state.
    const cb = currentConfirmCallback;
    currentConfirmCallback = null;

    // Run the callback; re-enable on failure so the sheet can be retried if
    // it didn't close (error path). On success closeAll() hides the button anyway.
  ...
  // --- GLOBAL UI BINDINGS ---
  // NOTE: the global FAB button was removed in v3.7 — each module now has its
  // own inline add button. The addMenuSheet is kept for legacy deeplinks only.
