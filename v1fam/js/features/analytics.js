...
// Module-level observer handle — disconnected and rebuilt on every initAnalytics()
// call so a re-init after an error doesn't stack a second observer on top.
let _analyticsThemeObserver = null;
...
export function initAnalytics() {
  ...
  // Disconnect any observer from a previous init pass before creating a new one
  if (_analyticsThemeObserver) {
    _analyticsThemeObserver.disconnect();
    _analyticsThemeObserver = null;
  }
  ...
  // The existing timeFilter <select> gets two new inputs for custom date ranges.
  let _analyticsFrom = '';  // 'YYYY-MM-DD'
  let _analyticsTo   = '';  // 'YYYY-MM-DD'

  // Show/hide the custom date range inputs when "customRange" is selected
  const rangeRow = document.getElementById('analyticsDateRangeRow');
  timeFilter?.addEventListener('change', () => {
    const isCustom = timeFilter.value === 'customRange';
    rangeRow?.classList.toggle('hidden', !isCustom);
    renderCharts(store);
  });
  document.getElementById('analyticsDateFrom')?.addEventListener('change', e => {
    _analyticsFrom = e.target.value;
    renderCharts(store);
  });
  document.getElementById('analyticsDateTo')?.addEventListener('change', e => {
    _analyticsTo = e.target.value;
    renderCharts(store);
  });
  ...
  function getStartDate() {
    ...
    else if (filter === 'customRange')  { return _analyticsFrom ? new Date(_analyticsFrom + 'T00:00:00') : new Date(0); }
    else return new Date(0);
    return d;
  }

  function getEndDate() {
    if (timeFilter?.value === 'customRange' && _analyticsTo) {
      return new Date(_analyticsTo + 'T23:59:59');
    }
    return new Date(8640000000000000); // max date — no upper bound for non-custom filters
  }
  ...
  function renderCharts(state) {
    ...
    // Normalize a possibly-Firestore-Timestamp value to a plain numeric epoch.
    // events.js Quick-Add used serverTimestamp() instead of Date.now(), producing
    // Timestamp objects that make (a.timestamp - b.timestamp) return NaN and
    // scramble the cashflow/MoM sort order.
    const toEpoch = v => {
      if (!v) return 0;
    ...
    sortedTxns.forEach(txn => {
      ...
      // Running liquid cash — mirrors store.getLiquidCash() by excluding investment-
      // tagged expenses (money moved to assets is a transfer, not a cash outflow).
      // Previously this used a plain income-minus-all-expenses formula, diverging
      // from the Hub and the export PDF for any user with investment transactions.
      const isInvestmentExpense =
        txn.type === 'expense' &&
        txn.tags?.some(t => t.toLowerCase() === 'investments');
      if (txn.type === 'income')        runningLiquid += amt;
      else if (!isInvestmentExpense)    runningLiquid -= amt;
      ...
      // Pie charts — respect time filter (both start and end bounds)
  ...
  _analyticsThemeObserver = new MutationObserver(() => {
    if (store.isLoaded) renderCharts(store);
  });
  _analyticsThemeObserver.observe(document.documentElement, { attributeFilter: ['class'] });
}
