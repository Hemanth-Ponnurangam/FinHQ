import { db, collection, addDoc, doc, updateDoc, deleteDoc, userCol } from '../firebase.js';
...
function xirrCalc(cashflows, dates) {
  ...
  for (let iter = 0; iter < 200; iter++) {
    ...
    // FIX: `rate * 1.5` fallback can compound to Infinity over 200 iterations
    // when cashflows are extreme or very short-dated, making the final isFinite
    // check the only guard (too late — NaN arithmetic has already propagated).
    // Clamp to a plausible economic range: −99.9% to +10,000% p.a.
    if (!isFinite(next) || next < -0.999 || next > 100) {
      // Halve toward zero rather than multiplying away
      rate = rate * 0.5;
    } else {
      rate = next;
    }
...
const CACHE_MAX  = 200;                       // max symbols stored
...
function setCache(c) {
  ...
  let cleaned = Object.fromEntries(
    Object.entries(c).filter(([, v]) => !v.fetchedAt || (now - v.fetchedAt) < CACHE_TTL)
  );
  // Cap total entries — drop oldest first
  const keys = Object.keys(cleaned);
  if (keys.length > CACHE_MAX) {
    keys.sort((a, b) => (cleaned[a].fetchedAt || 0) - (cleaned[b].fetchedAt || 0))
        .slice(0, keys.length - CACHE_MAX)
        .forEach(k => delete cleaned[k]);
  }
  try { localStorage.setItem(PRICE_KEY, JSON.stringify(cleaned)); } catch { /* storage full */ }
}

// ── Tax helpers ──────────────────────────────────────────────────────────────
// Finance Act 2024 rates (effective FY 2024-25, i.e. transfers on/after 23 Jul 2024):
//   Equity LTCG  → 12.5% above ₹1,25,000 exemption (Section 112A) — was 10% / ₹1L
//   Equity STCG  → 20%  (Section 111A) — was 15%
//   Non-equity LTCG → 12.5% WITHOUT indexation — was 20% WITH indexation
//   Debt MF / FD / PPF → slab rate, no LTCG concession
//
// The holding-period threshold for equity MFs was already updated in v3.5 (1 yr).
// This patch updates the rate and exemption constants that were missed.

// Equity LTCG exemption slab (Section 112A, FY 2024-25)
const EQUITY_LTCG_EXEMPTION = 125000;  // ₹1,25,000
const EQUITY_LTCG_RATE      = 0.125;   // 12.5%
const EQUITY_STCG_RATE      = 0.20;    // 20%  (Section 111A)
const NON_EQ_LTCG_RATE      = 0.125;   // 12.5% without indexation
const SLAB_RATE_APPROX      = 0.30;    // Slab-rate proxy for debt/FD/PPF
...
function taxHint(purchaseDateStr, pl, category) {
  ...
  const isEquity = category === 'Equity' || category === 'Mutual Fund';
  if (isLTCG) {
    if (isEquity) {
      const taxable = Math.max(0, pl - EQUITY_LTCG_EXEMPTION);
      const tax     = taxable * EQUITY_LTCG_RATE;
      return `${label}: ₹${Math.round(tax).toLocaleString('en-IN')} est. tax (12.5% above ₹1.25L — Sec 112A)`;
    }
    // Non-equity LTCG: 12.5% without indexation (from 23 Jul 2024)
    const tax = pl * NON_EQ_LTCG_RATE;
    return `${label}: ₹${Math.round(tax).toLocaleString('en-IN')} est. tax (12.5% — no indexation)`;
  }
  // STCG
  const rate = isEquity ? EQUITY_STCG_RATE : SLAB_RATE_APPROX;
  const label2 = isEquity ? '20% — Sec 111A' : '~30% slab';
  return `${label}: ₹${Math.round(pl * rate).toLocaleString('en-IN')} est. tax (${label2})`;
}
...
// ── Net Worth History ─────────────────────────────────────────────────────
// A lightweight snapshot is written to localStorage each time renderList()
// runs.  Only one entry per calendar month is kept (keyed 'YYYY-MM').
// This gives a growing time series without any backend writes.
const NW_HIST_KEY = 'finhq_nw_history_v1';
let   _nwChart    = null;   // Chart.js instance — destroyed on each re-render

function _loadNWHistory() {
  try { return JSON.parse(localStorage.getItem(NW_HIST_KEY) || '{}'); } catch { return {}; }
}
function _saveNWSnapshot(netWorth) {
  const hist = _loadNWHistory();
  const key  = new Date().toISOString().slice(0, 7);  // 'YYYY-MM'
  hist[key]  = Math.round(netWorth);
  // Keep last 24 months only — avoids unbounded growth
  const sorted = Object.keys(hist).sort();
  if (sorted.length > 24) sorted.slice(0, sorted.length - 24).forEach(k => delete hist[k]);
  try { localStorage.setItem(NW_HIST_KEY, JSON.stringify(hist)); } catch { /* storage full */ }
  return hist;
}
...
export function initWealth(ui) {
  ...
  let _activeRange   = '1d';
  // Pending average-cost recalculation result.  Stored here instead of as DOM
  // properties on avgCalcApplyBtn, which were never cleared between assets —
  // closing and reopening the detail sheet for a different asset could apply
  // the previous asset's avg/qty to the new one.
  let _pendingAvgCalc = null;
  ...
  form?.addEventListener('submit', async e => {
    ...
    if (qty <= 0) {
      let el = form.querySelector('.asset-err');
      if (!el) { el = document.createElement('p'); el.className='asset-err text-red-500 text-xs font-semibold mt-1 px-1'; form.querySelector('#saveAssetBtn')?.insertAdjacentElement('beforebegin', el); }
      el.textContent = 'Quantity must be greater than 0.';
  ...
  function recalcAvg() {
    ...
    // Store pending calculation in a module variable — not on the DOM element,
    // which persisted stale values across asset detail sheet open/close cycles.
    _pendingAvgCalc = { avg: newAvg, qty: newQty, inv: newInv };
  }
  ...
  document.getElementById('avgCalcApplyBtn')?.addEventListener('click', async () => {
    ...
    if (!_activeAsset || !_pendingAvgCalc) return;
    const { avg: newAvg, qty: newQty } = _pendingAvgCalc;
    ...
    try {
      ...
      _pendingAvgCalc = null; // consumed — clear so a re-open can't re-apply
  ...
  function recalcSell() {
    ...
    // Guard: a zero-qty asset would produce a ₹0 sale and a corrupt realisedGains
    // entry.  Hide the summary and show a warning instead of silently proceeding.
    if (qty <= 0) {
      document.getElementById('sellSummary')?.classList.add('hidden');
      const sellErr = document.getElementById('sellPrice');
      if (sellErr) {
        sellErr.classList.add('border-red-400');
        sellErr.setAttribute('placeholder', 'Qty is 0 — edit the asset first');
        setTimeout(() => { sellErr.classList.remove('border-red-400'); sellErr.setAttribute('placeholder', ''); }, 3000);
      }
      return;
    }
    ...
    const estTax  = rl > 0
      ? (tax.isLTCG
          ? ((_activeAsset.category === 'Equity' || _activeAsset.category === 'Mutual Fund')
              ? Math.max(0, rl - EQUITY_LTCG_EXEMPTION) * EQUITY_LTCG_RATE
              : rl * NON_EQ_LTCG_RATE)
          : rl * ((_activeAsset.category === 'Equity' || _activeAsset.category === 'Mutual Fund')
              ? EQUITY_STCG_RATE : SLAB_RATE_APPROX))
      : 0;
  ...
  document.getElementById('confirmSellBtn')?.addEventListener('click', async () => {
    ...
    // Guard: zero-qty asset — refuse before any Firestore write.
    if (!a.qty || a.qty <= 0) {
      alert('Cannot record a sale: this asset has a quantity of 0.\nEdit the asset to set the correct quantity first.');
      return;
    }
    if (!sp || sp <= 0) {
      const sellErr = document.getElementById('sellPrice');
      sellErr?.classList.add('border-red-400');
      sellErr?.setAttribute('placeholder', 'Enter a valid price');
      setTimeout(() => { sellErr?.classList.remove('border-red-400'); }, 3000);
      return;
    }
  ...
  function renderRealisedGains() {
    ...
    // Split gains by type and sign (losses don't attract tax)
    const eqStcg    = gains.filter(r=>!r.isLTCG && (r.category==='Equity'||r.category==='Mutual Fund')).reduce((s,r)=>s+(r.realisedPL>0?r.realisedPL:0),0);
    const nonEqStcg = gains.filter(r=>!r.isLTCG && r.category!=='Equity' && r.category!=='Mutual Fund').reduce((s,r)=>s+(r.realisedPL>0?r.realisedPL:0),0);
    const eqLtcg    = gains.filter(r=>r.isLTCG  && (r.category==='Equity'||r.category==='Mutual Fund')).reduce((s,r)=>s+(r.realisedPL>0?r.realisedPL:0),0);
    const nonEqLtcg = gains.filter(r=>r.isLTCG  && r.category!=='Equity' && r.category!=='Mutual Fund').reduce((s,r)=>s+(r.realisedPL>0?r.realisedPL:0),0);
    // FY 2024-25 tax estimates (Finance Act 2024)
    const taxEst =
      eqStcg    * EQUITY_STCG_RATE +                          // 20% (Sec 111A)
      nonEqStcg * SLAB_RATE_APPROX +                          // ~30% slab
      Math.max(0, eqLtcg - EQUITY_LTCG_EXEMPTION) * EQUITY_LTCG_RATE + // 12.5% above ₹1.25L
      nonEqLtcg * NON_EQ_LTCG_RATE;                           // 12.5% no indexation

    const stcg = eqStcg + nonEqStcg;
    const ltcg = eqLtcg + nonEqLtcg;
    ...
    document.getElementById('realisedTaxEst').textContent = `Est. tax this FY: ₹${Math.round(taxEst).toLocaleString('en-IN')} (Eq-STCG 20% + Eq-LTCG 12.5% above ₹1.25L — FA 2024)`;
  ...
  function openAssetDetail(asset) {
    _activeAsset    = asset;
    _activeRange    = '1d';
    _pendingAvgCalc = null;  // clear stale avg-calc result from previous asset
    ...
    // XIRR
    // FIX (wealth/medium): When inv=0 (ESOP grants, gifted shares), there's no
    // negative cashflow for Newton-Raphson to anchor on — xirrCalc returns null.
    // Show an explanatory tooltip instead of a bare dash so users aren't confused.
    let xirrText = '—';
    let xirrTitle = '';
    if (inv <= 0) {
      xirrTitle = 'XIRR unavailable: no purchase cost on record (e.g. ESOP grant, gift). Add a buy price to calculate returns.';
    ...
    const xirrEl = document.getElementById('assetDetailXIRR');
    if (xirrEl) {
      xirrEl.textContent = xirrText;
      xirrEl.title       = xirrTitle;
      xirrEl.style.cursor = xirrTitle ? 'help' : '';
    }
  ...
  // ── Render list ──────────────────────────────────────────────────────────
  _unsubs.push(store.subscribe(state => { if (state.isLoaded && list) renderList(state.assets); }, ['assets', 'debts']));

  // ── Net Worth History Chart render ────────────────────────────────────────
  function _renderNWHistoryChart(currentNW) {
    const hist    = _saveNWSnapshot(currentNW);
    const keys    = Object.keys(hist).sort();
    const card    = document.getElementById('nwHistoryCard');
    const canvas  = document.getElementById('wealthNWChart');
    const scroller= document.getElementById('nwChartScroller');
    const badge   = document.getElementById('nwHistoryDataPoints');

    if (!canvas || keys.length < 2) { card?.classList.add('hidden'); return; }
    card?.classList.remove('hidden');
    if (badge) badge.textContent = `${keys.length} month${keys.length > 1 ? 's' : ''}`;

    // Scale canvas width: min 320px, or ~52px per data point so bars don't crowd
    const minW = Math.max(320, keys.length * 52);
    if (scroller) scroller.style.minWidth = `${minW}px`;

    const isDark   = document.documentElement.classList.contains('dark');
    const gridCol  = isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)';
    const tickCol  = isDark ? '#9ca3af' : '#6b7280';
    const positive = '#3D9162';
    const negative = '#f87171';

    // Colour each point individually: green if ≥ previous, red if below
    const data   = keys.map(k => hist[k]);
    const pColors = data.map((v, i) => i === 0 ? positive : (v >= data[i - 1] ? positive : negative));
    ...
    _nwChart = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor:  isDark ? 'rgba(61,145,98,0.12)' : 'rgba(61,145,98,0.08)',
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: ctx => ` ₹${(ctx.raw || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}`,
            },
          },
        },
        scales: {
          x: { grid: { color: gridCol }, ticks: { font: { size: 9 }, color: tickCol, maxRotation: 45 } },
          y: {
            grid: { color: gridCol },
            ticks: {
              font: { size: 9 }, color: tickCol,
              callback: v => v >= 0
                ? '₹' + (v >= 1e7 ? (v / 1e7).toFixed(1) + 'Cr' : v >= 1e5 ? (v / 1e5).toFixed(1) + 'L' : (v / 1e3).toFixed(0) + 'k')
                : '-₹' + (Math.abs(v) >= 1e5 ? (Math.abs(v) / 1e5).toFixed(1) + 'L' : (Math.abs(v) / 1e3).toFixed(0) + 'k'),
            },
          },
        },
  ...
  function renderList(assets) {
    ...
    _renderNWHistoryChart(portVal - totalDebt);
...
export function initKiteImport(ui) {
  ...
  async function doImport() {
    ...
    if (!selH.length && !selG.length) {
      const errEl = document.getElementById('kiteConfirmBtn')?.closest('.space-y-3')?.querySelector('.kite-err')
        || (() => { const p = document.createElement('p'); p.className='kite-err text-red-500 text-xs font-semibold mt-2'; document.getElementById('kiteConfirmBtn')?.insertAdjacentElement('beforebegin', p); return p; })();
      errEl.textContent = 'Select at least one item to import.';
    ...
    try {
      ...
      for (const idx of selG) {
        const g   = gains[idx];
        const tax = taxLabel(g.purchaseDate?.toISOString?.(), g.category);
        await addDoc(userCol('realisedGains'), {
          name:         g.symbol,
          category:     g.category,
          qty:          g.qty,
      ...
      btn.disabled = false;
      btn.textContent = `⚠ Import failed — ${err.message.slice(0, 40)}`;
      setTimeout(() => { btn.textContent = 'Import Selected'; }, 5000);
    }
  }
  ...
  async function handleFiles(files) {
    ...
    if (!valid.length) {
      const pm = document.getElementById('kiteParseMsg');
      if (pm) { pm.textContent = 'Please select a Kite Tradebook XLSX or CSV file.'; pm.className = 'text-center text-[12px] text-red-500'; pm.classList.remove('hidden'); }
      return;
    }
