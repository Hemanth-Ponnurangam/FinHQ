...
import { db, collection, addDoc, userCol } from '../firebase.js';
...
export function initExport() {
  ...
  // ── CSV cell helper ───────────────────────────────────────────────────────
  // RFC-4180: wrap every field in double-quotes, escape embedded quotes by
  // doubling them.  Applies to ALL field types — numbers, booleans, and strings
  // that might contain commas, quotes, or newlines.  The importer's parseCSVRow()
  // already handles quoted fields, so this is a safe round-trip change.
  function csvCell(v) {
  ...
  exportCsvBtn?.addEventListener('click', () => {
    ...
    txns.forEach(t => {
      ...
      // Tags: join with '; ', then pass through csvCell so any embedded quotes
      // in individual tag values are properly doubled.
      ...
      csv += [
        ...
        csvCell(t.title),
        csvCell(t.type),
        t.amount || 0,          // numeric — no quoting needed
        csvCell(tagsStr),
        csvCell(t.label),
        csvCell(t.party),
        csvCell(t.eventId),
        csvCell(t.isRecurring),
        csvCell(t.splitGroupId),
      ].join(',') + '\n';
    });
  ...
  exportNwPdfBtn?.addEventListener('click', async () => {
    ...
    // events.js Quick-Add wrote serverTimestamp() (Firestore Timestamp object)
    // instead of Date.now() (number).  Subtracting a Timestamp from a number
    // yields NaN and corrupts the sort order of the PDF tables.
    const toEpoch = v => {
      if (!v) return 0;
    ...
    // Use the canonical getLiquidCash from store.js — excludes investment-tagged
    // expenses (transfers to the Wealth module) so this matches the Hub and the
    // Analytics liquid-cash trend exactly.  Previously this used a plain
    // income-minus-all-expenses formula, producing a third, different number.
    const liquidCash = store.getLiquidCash(store.transactions);
  ...
  importFileInput?.addEventListener('change', async () => {
    ...
    try {
      ...
      for (let i = 0; i < toAdd.length; i += BATCH) {
        await Promise.all(
          toAdd.slice(i, i + BATCH).map(t => addDoc(userCol('transactions'), t))
        );
      }
