...
function renderEventTags() {
  ...
  if (!container) return;
  container.innerHTML = _eventTags.map(tag =>
    `<span class="inline-flex items-center gap-1 bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 text-[10px] px-2 py-0.5 rounded-full font-semibold">
      ${escHtml(tag)}
      <button type="button" data-tag="${escHtml(tag)}" class="removeEventTag text-violet-400 hover:text-red-500 font-bold leading-none">×</button>
    </span>`
  ).join('');
  container.querySelectorAll('.removeEventTag').forEach(btn => {
    btn.addEventListener('click', () => {
      // Read the original tag from _eventTags by index rather than from data-tag,
      // so HTML-entity-encoded values don't mismatch the raw string in the array.
      const escaped = btn.dataset.tag;
      // Find the first entry whose escaped form matches (handles all special chars).
      _eventTags = _eventTags.filter(t => escHtml(t) !== escaped);
      renderEventTags();
    });
  });
}

// ── Quick-Add Tag helpers ──────────────────────────────────────────────────
function renderEqaTags() {
  const c = document.getElementById('eqaTagChips');
  if (!c) return;
  ...
  c.querySelectorAll('.removeEqaTag').forEach(btn => {
    btn.addEventListener('click', () => { _eqaTags = _eqaTags.filter(t => t !== btn.dataset.tag); renderEqaTags(); });
  });
}
...
function openEventQuickAdd(event) {
  ...
  const partyLabel = document.getElementById('eqaPartyLabel');
  const recurEl    = document.getElementById('eqaRecurring');
  const tagInput   = document.getElementById('eqaTagInput');
...
function openEventCreateSheet(event = null) {
  ...
  if (event) {
    ...
    // Tags
    _eventTags = event.tags ? [...event.tags] : [];
    renderEventTags();
...
export function initEvents(ui) {
  ...
  // ── Quick-Add tag input listener ──────────────────────────────
  document.getElementById('eqaTagInput')?.addEventListener('keydown', (e) => {
    if (e.key !== 'Enter' && e.key !== ',') return;
    e.preventDefault();
    const val = e.target.value.trim().replace(/,+$/, '');
    if (val && !_eqaTags.includes(val)) { _eqaTags.push(val); renderEqaTags(); }
    e.target.value = '';
  });

  // Recipient ↔ Payer label swap
  document.getElementById('eqaType')?.addEventListener('change', (e) => {
    const lbl = document.getElementById('eqaPartyLabel');
    const inp = document.getElementById('eqaParty');
    if (!lbl || !inp) return;
    if (e.target.value === 'income') { lbl.textContent = 'Payer / Source'; inp.placeholder = 'e.g. Employer, Client'; }
    else                             { lbl.textContent = 'Recipient';      inp.placeholder = 'e.g. Swiggy, Hotel, Driver'; }
  });

  // ── Event form tag input ───────────────────────────────────────
  const eventTagInput = document.getElementById('eventTagInput');
  eventTagInput?.addEventListener('keydown', (e) => {
    if (e.key !== 'Enter' && e.key !== ',') return;
    e.preventDefault();
    const val = eventTagInput.value.trim().replace(/,+$/, '');
    if (val && !_eventTags.includes(val)) {
      _eventTags.push(val);
      renderEventTags();
    }
    eventTagInput.value = '';
  ...
  eventForm?.addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      tags:      [..._eventTags],
      // FIX: serverTimestamp() produces a Firestore Timestamp object; all other
      // writers use Date.now() (a plain number).  Mixing types makes
      // (a.timestamp - b.timestamp) return NaN in analytics.js and export.js
      // sort operations, scrambling cashflow calculations and PDF tables.
      timestamp: Date.now(),
    };
    ...
    try {
      if (sheet?._editId) {
        ...
        await addDoc(userCol('events'), payload);
      }
      closeAllSheets();
    } catch (err) {
      console.error('Event save error:', err);
      const errEl = document.getElementById('eventForm')?.querySelector('.ev-err')
  ...
  // ── Event Bill Sheet ─────────────────────────────────────────
  let _currentBillEventId = null;

  function openEventBill(event) {
    _currentBillEventId = event.id;
    const state     = store;
    const txns      = (state.transactions || []).filter(t => t.eventId === event.id);
    const spend     = txns.filter(t => t.type === 'expense').reduce((s, t) => s + (t.amount || 0), 0);
    ...
    const accentMap = { trip: '#8b5cf6', wedding: '#ec4899', festival: '#f59e0b', birthday: '#f97316', party: '#6366f1', concert: '#06b6d4', sports: '#22c55e', conference: '#3b82f6', other: '#8b5cf6' };
    const accent    = accentMap[event.type] || '#8b5cf6';
    const accentBar = document.getElementById('eventBillAccentBar');
    if (accentBar) accentBar.style.background = accent;

    // Emoji + name + dates
    document.getElementById('eventBillEmoji').textContent = type.emoji || '🗓️';
    document.getElementById('eventBillName').textContent  = event.name || 'Event';
    document.getElementById('eventBillDates').textContent = formatDateRange(event.startDate, event.endDate);

    // Status badge
    const badgeEl = document.getElementById('eventBillStatusBadge');
    const statusRow = document.getElementById('eventBillStatusRow');
    if (status === 'active') {
      const left = daysLeft(event.endDate);
      if (badgeEl) badgeEl.textContent = 'Happening Now';
      if (statusRow) statusRow.innerHTML = `<span class="inline-block text-[9px] font-bold bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400 px-2 py-0.5 rounded-full">${left === 1 ? 'Last day!' : `${left}d left`}</span>`;
    } else if (status === 'upcoming') {
      const until = daysUntil(event.startDate);
      if (badgeEl) badgeEl.textContent = 'Upcoming';
      if (statusRow) statusRow.innerHTML = `<span class="inline-block text-[9px] font-bold bg-sky-100 text-sky-600 px-2 py-0.5 rounded-full">In ${until}d</span>`;
    } else {
      if (badgeEl) badgeEl.textContent = 'Past Event';
      if (statusRow) statusRow.innerHTML = `<span class="inline-block text-[9px] font-bold bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400 px-2 py-0.5 rounded-full">Done</span>`;
    }

    // Total spent
    const spentEl = document.getElementById('eventBillTotalSpent');
    if (spentEl) { spentEl.textContent = `₹${spend.toLocaleString('en-IN')}`; spentEl.className = `font-display text-2xl font-bold tabular-nums ${spend > budget && budget > 0 ? 'text-red-500' : 'text-forest-500'}`; }

    // Budget bar
    const budgetRow = document.getElementById('eventBillBudgetRow');
    if (budget > 0 && budgetRow) {
      const pct      = Math.min(100, Math.round((spend / budget) * 100));
      const over     = spend > budget;
      budgetRow.classList.remove('hidden');
      document.getElementById('eventBillBudgetPct').textContent   = `${pct}%`;
      document.getElementById('eventBillBudgetBar').style.width   = `${pct}%`;
      document.getElementById('eventBillBudgetBar').className      = `h-2 rounded-full transition-all ${over ? 'bg-red-500' : pct > 80 ? 'bg-amber-400' : 'bg-violet-500'}`;
      budgetRow.querySelector('span:first-child').textContent      = `₹${spend.toLocaleString('en-IN')} spent`;
      document.getElementById('eventBillBudgetTotal').textContent  = `of ₹${budget.toLocaleString('en-IN')}`;
    } else {
      budgetRow?.classList.add('hidden');
    }

    // Organizer
    const orgRow = document.getElementById('eventBillOrgRow');
    if (event.organizer && orgRow) {
      document.getElementById('eventBillOrganizer').textContent = event.organizer;
      orgRow.classList.remove('hidden');
    } else { orgRow?.classList.add('hidden'); }

    // Tags
    const tagsRow = document.getElementById('eventBillTagsRow');
    const tagsEl  = document.getElementById('eventBillTags');
    if (event.tags?.length && tagsRow && tagsEl) {
      tagsEl.innerHTML = event.tags.map(t =>
        `<span class="bg-violet-100/50 dark:bg-violet-900/20 text-violet-500/70 text-[9px] px-1.5 py-0.5 rounded font-semibold uppercase tracking-wide">${escHtml(t)}</span>`
      ).join('');
      tagsRow.classList.remove('hidden');
    } else { tagsRow?.classList.add('hidden'); }

    // Notes
    const notesRow = document.getElementById('eventBillNotesRow');
    if (event.notes && notesRow) {
      document.getElementById('eventBillNotes').textContent = event.notes;
      notesRow.classList.remove('hidden');
    } else { notesRow?.classList.add('hidden'); }

    // Transactions list
    const countEl  = document.getElementById('eventBillTxnCount');
    const listEl   = document.getElementById('eventBillTxnList');
    const emptyEl  = document.getElementById('eventBillNoTxns');
    ...
    if (listEl) {
      if (txns.length === 0) {
        ...
        const toEpoch = v => (v && typeof v.toMillis === 'function') ? v.toMillis() : (Number(v) || 0);
        const sorted = txns.slice().sort((a, b) => toEpoch(b.timestamp) - toEpoch(a.timestamp));
        listEl.innerHTML = sorted.map(t => {
          const isExp   = t.type === 'expense';
          const rawDate = t.date ? (t.date.includes('T') ? t.date : t.date + 'T00:00:00') : null;
          const dateStr = rawDate ? new Date(rawDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : '—';
          return `
            <div class="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700/50 last:border-0">
              <div class="min-w-0">
                <p class="text-[12px] font-semibold dark:text-white truncate">${escHtml(t.title || 'Untitled')}</p>
                <p class="text-[10px] text-gray-400">${dateStr}${t.party ? ` · ${escHtml(t.party)}` : ''}</p>
              </div>
              <p class="font-semibold text-sm flex-shrink-0 ml-3 tabular-nums ${isExp ? 'text-red-500' : 'text-forest-500'}">${isExp ? '-' : '+'}₹${Math.abs(t.amount || 0).toLocaleString('en-IN')}</p>
            </div>`;
        }).join('');
      }
    }

    // Show/hide add button for past events
    const addBtn = document.getElementById('eventBillAddTxnBtn');
    if (addBtn) addBtn.style.display = status === 'past' ? 'none' : 'block';

    _ui.openSheet(_ui.eventBillSheet);
  }

  // ── Edit pencil from bill sheet ───────────────────────────────
  document.getElementById('editFromEventBillBtn')?.addEventListener('click', () => {
    const event = store.events?.find(e => e.id === _currentBillEventId);
    if (event) openEventCreateSheet(event);
  });

  // ── Add txn from bill sheet ───────────────────────────────────
  document.getElementById('eventBillAddTxnBtn')?.addEventListener('click', () => {
    const event = store.events?.find(e => e.id === _currentBillEventId);
  ...
  document.getElementById('eventsList')?.addEventListener('click', (e) => {
    // Check specific buttons FIRST before falling through to card tap
    ...
    if (editBtn) {
      e.stopPropagation();
      const event = store.events?.find(ev => ev.id === editBtn.dataset.eventId);
      if (event) openEventCreateSheet(event);
      return;
    }
    if (quickAddBtn) {
      e.stopPropagation();
      const event = store.events?.find(ev => ev.id === quickAddBtn.dataset.eventId);
      if (event) openEventQuickAdd(event);
      return;
    }
    if (viewBtn) {
      e.stopPropagation();
      const event = store.events?.find(ev => ev.id === viewBtn.dataset.eventId);
      if (event) openEventBill(event);
      return;
    }
    // Tap on card body (not a button) → open bill sheet
    const card = e.target.closest('[data-event-id]');
    if (card) {
      const event = store.events?.find(ev => ev.id === card.dataset.eventId);
      if (event) openEventBill(event);
    }
  });
  ...
  qaForm?.addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      tags:        [event.name, ..._eqaTags.filter(t => t !== event.name)],
      party:       document.getElementById('eqaParty')?.value.trim() || '',
      isRecurring: document.getElementById('eqaRecurring')?.checked || false,
      eventId:     event.id,
      label:       '',
      notes:       '',
      timestamp:   Date.now(),  // FIX: was serverTimestamp() — Timestamp objects cause NaN in sort
    };
    ...
    try {
      await addDoc(userCol('transactions'), payload);
      // Clear per-entry fields for fast multi-entry; keep date & type
      const amtEl   = document.getElementById('eqaAmount');
      const titleEl = document.getElementById('eqaTitle');
      const partyEl = document.getElementById('eqaParty');
      const recurEl = document.getElementById('eqaRecurring');
