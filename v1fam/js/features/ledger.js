...
// ── XSS helper ─────────────────────────────────────────────────────────────
// FIX (security/critical): user-controlled strings (title, tags, event names)
// were interpolated directly into innerHTML — a self-XSS vector that becomes
// cross-user XSS the moment any document is shared (events, split groups).
// This function is called inside buildCardHTML for every card render.
...
// Active store-subscriber handles — collected so they can be unsubscribed
// before a re-init to prevent stacking duplicate callbacks on the store.
let _unsubs = [];
...
export function initLedger(ui) {
  ...
  _unsubs.push(store.subscribe(() => setTimeout(renderList, 0), ['transactions', 'events']));

  // FIX (ledger/ux): defensive initial render — if store already has transactions
  // (user navigated to Ledger after data loaded), render immediately without waiting
  // for the store subscribe's two chained setTimeouts.
  // This ensures the ledger is never blank on first visit even on slow event loops.
  if (store.isLoaded && store.transactions.length > 0) {
  ...
  const _subLedgerTags = store.subscribe(state => {
  ...
  // ── Recurring-type row: show when "Recurring" is checked ──────
  document.getElementById('txnIsRecurring')?.addEventListener('change', e => {
    document.getElementById('recurTypeRow')?.classList.toggle('hidden', !e.target.checked);
  });
  ...
  document.addEventListener('resetTxnForm', () => {
    ...
    // Reset recurring type selector and hide its row
    const recurTypeEl = document.getElementById('txnRecurType');
    if (recurTypeEl) recurTypeEl.value = 'fixedDate';
    document.getElementById('recurTypeRow')?.classList.add('hidden');
  ...
  const _subLedgerEvents = store.subscribe(state => {
    if (!state.isLoaded) return;
  ...
  function renderList() {
    ...
    _filtered.sort((a, b) => {
      if (sortKey === 'date')     return sortDir * ((new Date(a.date || 0)) - (new Date(b.date || 0)));
    ...
    if (_filtered.length === 0) {
      const emptyMsg = (store.transactions.length === 0)
        ? '<p class="text-center text-forest-400 py-12 text-sm">No transactions yet.<br><span class="text-xs text-forest-300">Tap the + button to add your first entry.</span></p>'
        : '<p class="text-center text-forest-400 py-10 text-sm">No results match your current filter or search.</p>';
  ...
  if (form) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      async function doSave() {
        const isNewRecurring = document.getElementById('txnIsRecurring')?.checked && !currentEditId;
        const basePayload = {
        title:       txnTitle,
        ...
        // FIX: assign a group ID on first creation so the parent and all 12 children
        // share a common recurGroupId.  Previously the children had no link back to the
        // parent, making bulk-cancellation impossible.
        ...(isNewRecurring ? { recurGroupId: `recur_${Date.now()}` } : {}),
      ...
      try {
        ...
        if (isSplitMode && !currentEditId) {
          ...
          else               await addDoc(userCol('transactions'), payload);
        }
        ...
        if (basePayload.isRecurring && !currentEditId && !isSplitMode) {
          ...
          const recurType     = document.getElementById('txnRecurType')?.value || 'fixedDate';
          const [ry, rmo, rd] = txnDateRaw.split('-').map(Number);

          // ── Date-anchor helpers ────────────────────────────────────────
          const isWeekend = d => { const day = d.getDay(); return day === 0 || day === 6; };
          const lastWorkingDayOf = (year, mo0) => {
            const d = new Date(year, mo0 + 1, 0);   // last calendar day
            while (isWeekend(d)) d.setDate(d.getDate() - 1);
            return d;
          };
          const targetDateFor = m => {
            const raw  = rmo - 1 + m;
            const tY   = ry + Math.floor(raw / 12);
            const tM   = raw % 12;
            if (recurType === 'lastWorkingDay') return lastWorkingDayOf(tY, tM);
            if (recurType === 'lastDay')        return new Date(tY, tM + 1, 0);
            // fixedDate: clamp to month-end if shorter than the original day
            const probe = new Date(tY, tM, rd);
            return probe.getMonth() !== tM ? new Date(tY, tM + 1, 0) : probe;
          };

          // Build all 12 payloads then write in parallel — a sequential for-await
          // previously left orphan entries on Firestore errors (4 of 12 written, no feedback).
          const recurPayloads = [];
          for (let m = 1; m <= RECUR_MONTHS; m++) {
            const fd  = targetDateFor(m);
            const fds = `${fd.getFullYear()}-${String(fd.getMonth()+1).padStart(2,'0')}-${String(fd.getDate()).padStart(2,'0')}`;
            recurPayloads.push({
              ...basePayload,
              amount:       rawAmount,
              tags:         currentTags,
              ...
              recurType,
              // Inherit the group ID from the parent — this is the link that
              // makes bulk-cancellation possible (find all where recurGroupId === X)
              ...(basePayload.recurGroupId ? { recurGroupId: basePayload.recurGroupId } : {}),
              date:         fds + 'T00:00:00',
              timestamp:    new Date(fds + 'T00:00:00').getTime(),
              isRecurring:  true
            });
          }

          const results = await Promise.allSettled(
            recurPayloads.map(p => addDoc(userCol('transactions'), p))
          );
          const failed = results.filter(r => r.status === 'rejected').length;
          if (failed > 0) {
            // Don't throw — the primary transaction already saved successfully.
            // Surface a non-blocking warning so the user can retry.
            console.error('Recurring entries: %d of %d failed', failed, RECUR_MONTHS,
              results.filter(r => r.status === 'rejected').map(r => r.reason));
            const saved = RECUR_MONTHS - failed;
            setTimeout(() => {
              alert(`✓ Transaction saved.\n⚠ Only ${saved}/${RECUR_MONTHS} recurring entries were created (${failed} failed). Check your connection and try again.`);
            }, 350); // after sheet animation completes
          }
        }
  ...
  function openReceiptSheet(txn) {
    ...
    if (tagsContainer) {
      tagsContainer.innerHTML = displayTags.map(t =>
        `<span class="bg-forest-100 dark:bg-gray-700 text-forest-700 dark:text-gray-300 text-[9px] px-2 py-0.5 rounded font-semibold uppercase">${escHtml(t)}</span>`
      ).join('');
    }
    ...
    // ── Cancel-series button (bulk delete all transactions sharing recurGroupId) ──
    const cancelSeriesRow = document.getElementById('receiptCancelSeriesRow');
    const cancelSeriesBtn = document.getElementById('receiptCancelSeriesBtn');
    if (cancelSeriesRow && cancelSeriesBtn) {
      if (txn.recurGroupId) {
        cancelSeriesRow.classList.remove('hidden');
        cancelSeriesBtn.onclick = () => {
          const groupId = txn.recurGroupId;
          const siblings = store.transactions.filter(t =>
            t.recurGroupId === groupId && t.id !== txn.id
          );
          const futureOnly = siblings.filter(t => {
            const d = new Date((t.date || '').split('T')[0] + 'T00:00:00');
            return d > new Date();
          });
          const label = futureOnly.length === 0
            ? 'No future entries in this series.'
            : `Delete ${futureOnly.length} future recurring entr${futureOnly.length === 1 ? 'y' : 'ies'}?`;
          ui.showConfirm(
            'Cancel Recurring Series',
            `${label}\n\nThis transaction itself will not be deleted — only upcoming unpaid entries.`,
            async () => {
              if (futureOnly.length === 0) return;
              const results = await Promise.allSettled(
                futureOnly.map(t => deleteDoc(doc(db, 'transactions', t.id)))
              );
              const failed = results.filter(r => r.status === 'rejected').length;
              if (failed > 0) console.error(`Cancel series: ${failed} deletes failed`);
            }
          );
        };
      } else {
        cancelSeriesRow.classList.add('hidden');
      }
    }
  ...
  document.getElementById('editFromReceiptBtn')?.addEventListener('click', () => {
    ...
    if (document.getElementById('txnIsRecurring')) {
      document.getElementById('txnIsRecurring').checked = txn.isRecurring || false;
      // Reveal/restore the recurring-type selector when editing a recurring entry
      const recurTypeEl = document.getElementById('txnRecurType');
      const recurTypeRow = document.getElementById('recurTypeRow');
      if (txn.isRecurring) {
        recurTypeRow?.classList.remove('hidden');
        if (recurTypeEl && txn.recurType) recurTypeEl.value = txn.recurType;
      } else {
        recurTypeRow?.classList.add('hidden');
      }
    }
    // Party
