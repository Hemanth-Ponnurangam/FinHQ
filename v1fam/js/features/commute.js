    ...
    return `
      <div data-id="${log.id}" class="edit-service cursor-pointer bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card border border-forest-50/50 dark:border-gray-700 flex items-center gap-3 active:scale-[0.98]">
        ...
        <div class="flex-1 min-w-0">
          <p class="font-semibold text-sm dark:text-white">${meta.label}${log.vehicle ? ` · <span class="font-normal text-gray-400">${escHtml(log.vehicle)}</span>` : ''}</p>
          <p class="text-xs text-gray-400 truncate">${dateStr}${log.notes ? ` · ${escHtml(log.notes)}` : ''}</p>
        </div>
...
export function initCommute(ui) {
  ...
  const plateStyleMap = {
    private:    { bg: '#ffffff', text: '#111111' },
    commercial: { bg: '#FFFF00', text: '#111111' },
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      if (odo <= 0 || liters <= 0 || cost <= 0) {
        ...
        let el = fuelForm?.querySelector('.fuel-err');
        if (!el && fuelForm) { el = document.createElement('p'); el.className='fuel-err text-red-500 text-xs font-semibold mt-1 px-1'; fuelForm.querySelector('#saveFuelBtn')?.insertAdjacentElement('beforebegin', el); }
        if (el) { el.textContent = `Odometer, ${meta.unit === 'kWh' ? 'energy' : 'volume'} and cost must all be greater than 0.`; clearTimeout(el._t); el._t = setTimeout(() => { el.textContent=''; }, 4000); }
        return;
      }
      ...
      async function doSaveFuel() {
        try {
          ...
          if (currentEditId) {
            ...
            await addDoc(userCol('fuel'), payload);
            if (autoLog) {
    ...
    serviceForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (currentServiceEditId) {
          ...
          await addDoc(userCol('serviceLog'), payload);
        }
      } catch (err) { console.error(err); } finally { ui.closeAll(); }
    });
  ...
  // ── Store subscription ───────────────────────────────────────────────────
  const _subCommute = store.subscribe(state => {
    ...
    for (let i = sortedLogs.length - 1; i >= 0; i--) {
      ...
      const plateBadge = log.plate
        ? `<span class="font-mono font-black text-[9px] px-1.5 py-0.5 rounded ${plateColors[log.plateType || 'private'] || plateColors.private}">${escHtml(log.plate)}</span>`
        : '';
        ...
        <div data-id="${log.id}" class="edit-fuel cursor-pointer bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card border border-forest-50/50 dark:border-gray-700 active:scale-[0.98]">
          ...
          <div class="flex justify-between items-center mt-1">
            <span class="text-xs text-gray-400">
              ...
              ${log.vehicle ? `<span class="ml-1 text-gray-300 dark:text-gray-600">|</span> ${escHtml(log.vehicle)}` : ''}
            </span>
            ${statsBadge}
          </div>
        </div>`);
