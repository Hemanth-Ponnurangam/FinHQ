    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (basePayload.isRecurring && !currentEditId && !isSplitMode) {
          ...
          // FIX (ledger/high): Date.setMonth() clamps day-of-month — Jan 31 + 1 month
          // becomes Feb 28 silently, misaligning every future recurring entry.
          // Pattern: compute target year/month explicitly, then clamp day to month-end.
          const [ry, rmo, rd] = txnDateRaw.split('-').map(Number);

          // Build all 12 payloads first, then write them in parallel via
          // Promise.allSettled. A sequential for-await loop previously aborted
          // mid-way on a Firestore error, leaving partial entries (e.g. 4 of 12)
          // with no feedback to the user.
          const recurPayloads = [];
          for (let m = 1; m <= RECUR_MONTHS; m++) {
            const rawTarget  = rmo - 1 + m;           // 0-indexed month offset
            const tYear      = ry + Math.floor(rawTarget / 12);
            const tMo        = rawTarget % 12;         // 0-indexed
            // Does the original day exist in this month?
            const probe      = new Date(tYear, tMo, rd);
            const actualDay  = probe.getMonth() !== tMo
              ? new Date(tYear, tMo + 1, 0).getDate() // last day of that month
              : rd;
            const futureDate    = new Date(tYear, tMo, actualDay);
