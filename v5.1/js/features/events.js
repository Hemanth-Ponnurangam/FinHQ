  ...
  function openEventBill(event) {
    ...
    if (event.tags?.length && tagsRow && tagsEl) {
      tagsEl.innerHTML = event.tags.map(t =>
        `<span class="bg-violet-100/50 dark:bg-violet-900/20 text-violet-500/70 text-[9px] px-1.5 py-0.5 rounded font-semibold uppercase tracking-wide">${t}</span>`
      ).join('');
    ...
    if (listEl) {
      if (txns.length === 0) {
        ...
        // Sort by date desc
        const sorted = txns.slice().sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
        listEl.innerHTML = sorted.map(t => {
          ...
          return `
            <div class="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700/50 last:border-0">
              <div class="min-w-0">
                <p class="text-[12px] font-semibold dark:text-white truncate">${t.title || 'Untitled'}</p>
                <p class="text-[10px] text-gray-400">${dateStr}${t.party ? ` · ${t.party}` : ''}</p>
              </div>
