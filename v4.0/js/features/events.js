...
function renderEventTags() {
  ...
  container.innerHTML = _eventTags.map(tag =>
    `<span class="inline-flex items-center gap-1 bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 text-[10px] px-2 py-0.5 rounded-full font-semibold">
      ${tag}
      <button type="button" data-tag="${tag}" class="removeEventTag text-violet-400 hover:text-red-500 font-bold leading-none">×</button>
    </span>`
  ).join('');
  container.querySelectorAll('.removeEventTag').forEach(btn => {
    btn.addEventListener('click', () => {
      _eventTags = _eventTags.filter(t => t !== btn.dataset.tag);
      renderEventTags();
    });
  });
}
