...
export function initLedger(ui) {
  ...
  store.subscribe(() => renderList(), ['transactions', 'events']);
  ...
  const PAGE_SIZE = 50;
  let _filtered     = [];   // full sorted+filtered result set
  let _rendered     = 0;    // how many cards are in the DOM
  let _sentinel     = null; // the "load more" div watched by the observer
  let _observer     = null; // IntersectionObserver instance
