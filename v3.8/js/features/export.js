...
import {
  getFirestore, getDocs, query, orderBy
...
export function initExport() {
  ...
  exportCsvBtn?.addEventListener('click', () => {
    ...
    if (txns.length === 0) return alert('No transactions in the selected date range.');
  ...
  exportAssetsBtn?.addEventListener('click', () => {
    if (!store.isLoaded) return alert('Data still loading. Please try again.');
    if (!store.assets.length) return alert('No assets tracked yet.');
  ...
  exportDebtsBtn?.addEventListener('click', () => {
    if (!store.isLoaded) return alert('Data still loading. Please try again.');
    if (!store.debts.length) return alert('No debts tracked yet.');
  ...
  // budgets and goals are NOT kept in the store (no always-on listener).
  // We do a one-shot getDocs() only when the user actually clicks Export.
  exportBudgetsBtn?.addEventListener('click', async () => {
    const btn = exportBudgetsBtn;
    btn.disabled = true;
    const labelEl = btn.querySelector('p.font-semibold');
    const origLabel = labelEl?.textContent;
    if (labelEl) labelEl.textContent = 'Fetching…';

    try {
      const [budgetsSnap, goalsSnap] = await Promise.all([
      ...
      // Budgets
      csv += '=== BUDGETS ===\n';
      csv += 'Category,Monthly Limit (INR)\n';
      if (budgets.length) {
        budgets.forEach(b => {
          const cat = `"${(b.category || '').replace(/"/g, '""')}"`;
          csv += `${cat},${b.amount || 0}\n`;
        });
      } else { csv += '(no budgets set)\n'; }

      // Goals
      csv += '\n=== GOALS ===\n';
      csv += 'Goal Name,Target (INR),Saved (INR),Remaining (INR),Deadline\n';
      if (goals.length) {
        goals.forEach(g => {
          const name      = `"${(g.name || '').replace(/"/g, '""')}"`;
          const remaining = Math.max(0, (g.target || 0) - (g.saved || 0));
          const deadline  = g.deadline || '';
          csv += `${name},${g.target || 0},${g.saved || 0},${remaining},${deadline}\n`;
        });
      } else { csv += '(no goals set)\n'; }

      // Recurring / SIP (always in store via Tier 2)
      csv += '\n=== RECURRING PAYMENTS ===\n';
      csv += 'Name,Amount (INR),Category,Billing Date,Type,Annual Total (INR)\n';
      if (store.recurring?.length) {
        store.recurring.forEach(r => {
          const name   = `"${(r.name || '').replace(/"/g, '""')}"`;
          const annual = (r.amount || 0) * 12;
          csv += `${name},${r.amount || 0},${r.category || ''},${r.billingDate || ''},${r.type || 'expense'},${annual}\n`;
        });
      } else { csv += '(no recurring payments set)\n'; }

      downloadCSV(csv, `FinHQ_Budgets_Goals_Recurring_${today}.csv`);
    } catch (err) {
      console.error('Export budgets/goals failed:', err);
  ...
  exportPdfBtn?.addEventListener('click', async () => {
    ...
    catch (err) { setBtnLoading(btn, false, origLabel); return alert(err.message); }

    if (!store.isLoaded) { setBtnLoading(btn, false, origLabel); return alert('Data still loading.'); }
    ...
    if (!txns.length) { setBtnLoading(btn, false, origLabel); return alert('No transactions found for this period.'); }
