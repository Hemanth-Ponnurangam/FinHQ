...
export function initLedger(ui) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (isSplitMode && !currentEditId) {
          ...
          // BUG 3: pending-flush block above already moved tagInput.value into currentTags;
          // the old fallback that re-read txnTagInput.value was dead code causing a double-read.
        ...
        if (basePayload.isRecurring && !currentEditId && !isSplitMode) {
          ...
          const baseLabel    = document.getElementById('txnLabel')?.value.trim() || '';
          for (let m = 1; m <= RECUR_MONTHS; m++) {
            ...
            await addDoc(collection(db, 'transactions'), {
              ...
              amount:      rawAmount,
              tags:        currentTags,
              label:       (baseLabel ? baseLabel + ' ' : '') + `(Auto-recurring +${m}mo)`,
  ...
  list?.addEventListener('click', (e) => {
    ...
    // BUG FIX / GAP 2: Split editing — re-enable toggle and load existing split children if it's a split parent
    if (splitToggle) {
      if (txn.splitGroupId) {
        // Split child: disable re-splitting but allow all other fields (title, amount, tags, date) to be edited
        splitToggle.disabled = true;
        // Show a clear info notice so users know the form IS editable
        ...
        splitNotice.textContent = 'Split entry — you can edit title, amount, tags and date. Re-splitting is disabled for children.';
