...
export function initLedger(ui) {
  ...
  receiptInput?.addEventListener('change', () => {
    ...
    reader.onload = (ev) => {
      currentReceipt = ev.target.result; // base64 data URL
  ...
  document.getElementById('bulkRetagBtn')?.addEventListener('click', () => {
    if (selectedIds.size === 0) return;
    const newTag = prompt(`Add tag to ${selectedIds.size} transaction${selectedIds.size > 1 ? 's' : ''}:\n(Leave blank to cancel)`);
  ...
  function renderList() {
    ...
    filtered.forEach(txn => {
      ...
      // GAP FIX: show ALL tags, not just first
      const tagBadges = (txn.tags || []).map(tag =>
      ...
      const customLabel = txn.label
        ? `<span class="bg-gray-100 dark:bg-gray-700 text-[9px] px-1.5 py-0.5 rounded ml-1">${txn.label}</span>`
        : '';
      ...
      list.innerHTML += `
        <div data-id="${txn.id}" class="edit-card cursor-pointer bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card border border-forest-50/50 dark:border-gray-700 flex justify-between items-center active:scale-[0.98] ${isSelected ? 'ring-2 ring-forest-900 dark:ring-forest-400' : ''}">
          ...
          <div class="flex-1 min-w-0 mr-3">
            <p class="font-semibold text-forest-900 dark:text-white flex items-center flex-wrap line-clamp-1">${txn.title || 'Untitled'} ${customLabel}${splitBadge}${recurBadge}${receiptIcon}</p>
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (isSplitMode && !currentEditId) {
          ...
          const finalTags = currentTags.length > 0
            ? currentTags
            : (document.getElementById('txnTagInput')?.value.trim() ? [document.getElementById('txnTagInput').value.trim()] : []);
        ...
        // GAP FIX: auto-generate next month entry for recurring transactions
        if (basePayload.isRecurring && !currentEditId && !isSplitMode) {
          const nextMonth = new Date(txnDate + 'T00:00:00');
          ...
          await addDoc(collection(db, 'transactions'), {
            ...
            tags:        currentTags,
            label:       document.getElementById('txnLabel')?.value.trim() || null,
            date:        nextDateStr + 'T00:00:00',
            timestamp:   new Date(nextDateStr + 'T00:00:00').getTime(),
            label:       (document.getElementById('txnLabel')?.value.trim() || '') + ' (Auto-recurring)',
            isRecurring: true
          });
        }
  ...
  // ── Click Delegation (edit + bulk select) ─────────────────────
  list?.addEventListener('click', (e) => {
    ...
    if (txn.date) {
      const datePart = txn.date.split('T')[0];
      document.getElementById('txnDate').value = datePart;
    }
    ...
    // BUG FIX: Split editing — re-enable toggle and load existing split children if it's a split parent
    if (splitToggle) {
      if (txn.splitGroupId) {
        // This is a split child — allow editing as a plain txn, disable re-splitting
