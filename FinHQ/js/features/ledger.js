...
export function initLedger(ui) {
  ...
  function renderList() {
    ...
    filtered.forEach(txn => {
      ...
      const customLabel = txn.eventId
        ? (() => {
            const ev = (store.events || []).find(e => e.id === txn.eventId);
            return ev
              ? `<span class="bg-violet-100 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 text-[9px] px-1.5 py-0.5 rounded ml-1">🎉 ${ev.name}</span>`
              : '';
          })()
        : '';
