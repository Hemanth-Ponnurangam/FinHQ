...
export function initDebt(ui) {
  ...
  // ── Global Part-Payment Simulator (across all loans) ──────────
  function globalPartPaymentSimulator() {
    ...
    const rows = debts.map(d => {
      ...
      return { name: d.name, outstanding, emi, months, totalInterest, mode: d.loanMode || 'standard', rate };
    });

    const totalOutstanding = rows.reduce((s, r) => s + r.outstanding, 0);
    const totalMonthlyEMI  = rows.reduce((s, r) => s + r.emi, 0);
    const totalInterest    = rows.reduce((s, r) => s + r.totalInterest, 0);

    return `
      <div class="space-y-3 text-xs">
        <div class="p-3 bg-gray-50 dark:bg-gray-700 rounded-xl">
          <p class="font-semibold text-xs dark:text-white mb-2 text-gray-700">📋 All Loans Summary</p>
          ${rows.map(r => `
            <div class="flex justify-between py-1 border-b dark:border-gray-600 last:border-0">
              <span class="text-gray-600 dark:text-gray-300">${r.name}
                ${r.mode === 'moratorium' ? '<span class="text-orange-500 ml-1">[Moratorium]</span>' : ''}
                ${r.mode === 'bullet' ? '<span class="text-purple-500 ml-1">[Bullet]</span>' : ''}
              </span>
              <span class="font-semibold dark:text-white">${fmtINR(r.outstanding)} · ${r.rate}%</span>
            </div>`).join('')}
          <div class="flex justify-between pt-2 font-bold dark:text-white">
            <span>Total Outstanding</span><span class="text-red-500">${fmtINR(totalOutstanding)}</span>
          </div>
          <div class="flex justify-between font-bold dark:text-white">
            <span>Total Monthly EMI</span><span class="text-blue-500">${fmtINR(totalMonthlyEMI)}</span>
          </div>
          <div class="flex justify-between font-bold dark:text-white">
            <span>Total Future Interest</span><span class="text-orange-500">${fmtINR(totalInterest)}</span>
          </div>
        </div>
        <div class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
          <p class="text-[10px] text-blue-600 dark:text-blue-400 font-semibold uppercase mb-2">🎯 Global Part-Payment What-If</p>
          <div class="flex gap-2 items-center mb-2">
            <input type="number" id="globalPartPayInput" class="flex-1 p-2 rounded-lg bg-white dark:bg-gray-700 dark:text-white text-sm outline-none border border-blue-200 dark:border-gray-600" placeholder="Lump-sum amount (₹)">
            <button type="button" id="globalPartPayBtn" class="px-3 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold whitespace-nowrap">Compare</button>
          </div>
          <div id="globalPartPayResult" class="hidden space-y-2 mt-2"></div>
        </div>
      </div>`;
  }
  ...
  document.getElementById('openStrategyBtn')?.addEventListener('click', () => {
    ...
    if (partPayContainer) {
      ...
      document.getElementById('globalPartPayBtn')?.addEventListener('click', () => {
        ...
        if (!resultDiv || lumpSum <= 0) { resultDiv?.classList.add('hidden'); return; }
        ...
        // Per-loan impact
        const perLoanRows = debts.map(d => {
          const outstanding = d.principal - (d.paid || 0);
          const rate = _currentRate(d);
          const emi  = currentEMI(d);
          const base = calcPayoff(outstanding, rate, emi);
          const sim  = calcPayoff(Math.max(0, outstanding - lumpSum / debts.length), rate, emi);
          return { name: d.name, monthsSaved: base.months - sim.months, interestSaved: base.totalInterest - sim.totalInterest };
        });
        ...
        resultDiv.innerHTML = `
          <div class="p-2 bg-orange-50 dark:bg-orange-900/20 rounded-lg text-[10px] text-orange-700 dark:text-orange-300 mb-1">
            Baseline: ${baseLine.months} months · ${fmtINR(baseLine.totalInterest)} total interest
          </div>
          ${results.map((r, i) => `
            <div class="p-2 bg-${r.colorCls}-50 dark:bg-${r.colorCls}-900/20 rounded-lg text-xs space-y-1 ${i === bestIdx ? 'ring-2 ring-' + r.colorCls + '-400' : ''}">
              <div class="flex items-center justify-between">
                <p class="font-semibold text-${r.colorCls}-700 dark:text-${r.colorCls}-300">${r.label}${i === bestIdx ? ' 🏆' : ''}</p>
                ${i === bestIdx ? `<span class="text-[9px] bg-${r.colorCls}-100 text-${r.colorCls}-600 px-1.5 py-0.5 rounded font-semibold">Best</span>` : ''}
              </div>
              <div class="flex justify-between text-gray-600 dark:text-gray-300">
  ...
  function _renderStandardTable(debt, tbody, extraLumpSum) {
    ...
    tbody.innerHTML += `<div class="mt-4 p-3 bg-red-50 dark:bg-gray-700 rounded-xl flex justify-between text-sm">
  ...
  function _renderMoratoriumTable(debt, tbody, extraLumpSum) {
    ...
    tbody.innerHTML += `<div class="text-[10px] font-bold text-orange-600 uppercase tracking-widest py-2 border-b dark:border-gray-600">📚 Moratorium Phase — Interest Accruing</div>`;
    for (let i = 0; i < morMonths; i++) {
      const interest = balance * r;
      totalInterest += interest;
      balance += interest;
    ...
    if (grownPrincipal <= 0) {
      tbody.innerHTML += '<p class="text-center text-green-600 font-semibold py-4">🎉 Fully paid off!</p>';
      return;
    }
    const emi = calculateEMI(grownPrincipal, debt.rate || 0, Math.max(1, repayMonths));
    let repayBal = grownPrincipal;
    tbody.innerHTML += `<div class="text-[10px] font-bold text-green-600 uppercase tracking-widest py-2 border-b dark:border-gray-600 mt-2">✅ Repayment Phase — EMI ₹${Math.round(emi).toLocaleString('en-IN')}/mo</div>`;
    ...
    tbody.innerHTML += `<div class="mt-4 p-3 bg-orange-50 dark:bg-gray-700 rounded-xl">
  ...
  function _renderBulletTable(debt, tbody) {
    ...
    tbody.innerHTML += `<div class="text-[10px] font-bold text-purple-600 uppercase tracking-widest py-2 border-b dark:border-gray-600">🏅 Bullet/Gold Loan — Interest Only Until Maturity</div>`;
    ...
    tbody.innerHTML += `<div class="mt-4 p-3 bg-purple-50 dark:bg-gray-700 rounded-xl space-y-1">
  ...
  store.subscribe(state => {
    ...
    state.debts.forEach(debt => {
      ...
      const emiLabel = loanMode === 'moratorium' && liveEMI === 0
        ...
        : `<p>EMI: ₹${Math.round(liveEMI).toLocaleString('en-IN')}/mo${loanMode === 'bullet' ? ' (interest only)' : ''}</p>`;
      ...
      list.innerHTML += `
        <div data-id="${debt.id}" class="edit-card cursor-pointer bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card border border-red-50 dark:border-gray-700 flex flex-col gap-2 active:scale-[0.98]">
          <div class="flex justify-between items-center">
            <p class="font-semibold text-forest-900 dark:text-white flex items-center flex-wrap">${debt.name} ${rateLabel}${typeLabel}${modeLabel}</p>
    ...
    if (e.target.closest('.log-emi-btn')) {
      ...
      if (debt.lastEmiLogged) {
        ...
        if (lastDate.getMonth() === now.getMonth() && lastDate.getFullYear() === now.getFullYear()) {
          if (!confirm(`"${debt.name}" EMI was already logged this month.\n\nLog again?`)) return;
        }
      }
      ...
      try {
        ...
        await addDoc(collection(db, 'transactions'), {
          title: `${debt.name} — EMI`, amount: Math.round(liveEmi), type: 'expense',
          date: now.toISOString(), timestamp: now.getTime(),
          tags: [debt.loanType ? `${debt.loanType} Loan` : 'Loan'], label: 'EMI Payment'
        });

        const newPaid = Math.min(debt.principal, (debt.paid || 0) + principalPortion);
        await updateDoc(doc(db, 'debts', debt.id), { paid: newPaid, lastEmiLogged: now.toISOString() });

        btn.innerText = '✓ Logged';
        btn.className = btn.className.replace('red', 'green').replace('log-emi-btn ', '');
      } catch (err) {
        console.error('Log EMI failed:', err);
        btn.innerText = 'Log EMI'; btn.disabled = false;
      }
      return;
    }
