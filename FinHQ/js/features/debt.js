...
export function initDebt(ui) {
  ...
  // QA Fix: Clarify ambiguous UI label dynamically
  const paidLabel = document.querySelector('label[for="debtPaid"]');
  if (paidLabel) paidLabel.innerText = "Total Principal Repaid";
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      if (paid < 0 || paid > principal) return alert("Paid amount cannot exceed principal.");
  ...
  // Render Strategy Sheet with explicit instructions
  document.getElementById('openStrategyBtn')?.addEventListener('click', () => {
    ...
    avalancheDiv.innerHTML = avalanche.map((d, i) => `
      <div class="flex justify-between text-sm items-center p-2 ${i===0 ? 'bg-blue-100 dark:bg-blue-900/30 rounded-lg' : ''}">
        <span class="font-semibold dark:text-white">${i+1}. ${d.name} ${i===0 ? '🎯 (Target Extra Here)' : ''}</span> 
        <span class="text-red-500 font-bold">${d.rate}%</span>
      </div>`).join('');
    
    const snowball = [...debts].sort((a, b) => (a.principal - (a.paid||0)) - (b.principal - (b.paid||0)));
    snowballDiv.innerHTML = snowball.map((d, i) => `
      <div class="flex justify-between text-sm items-center p-2 ${i===0 ? 'bg-green-100 dark:bg-green-900/30 rounded-lg' : ''}">
        <span class="font-semibold dark:text-white">${i+1}. ${d.name} ${i===0 ? '🎯 (Target Extra Here)' : ''}</span> 
  ...
  // QA Fix: Prevent Amortization Infinite Loop
  function showAmortization(debt) {
    ...
    let emi = debt.emi;
    let month = 1; let totalInterest = 0;

    // Strict safety bound: Will never spin endlessly if EMI/Rate inputs are malformed
    const maxMonths = debt.tenure + 2; 
    ...
    while (balance > 0.5 && month <= maxMonths) {
      ...
      // Failsafe for 0% loans or bad data where EMI doesn't cover interest
      if (principalPaid <= 0) principalPaid = balance / debt.tenure; 
  ...
  store.subscribe(state => {
    ...
    list.innerHTML = ''; let localTotal = 0;

    if (state.debts.length === 0) {
      list.innerHTML = '<p class="text-center text-forest-400 py-10 text-sm col-span-full">No debts registered.</p>';
      ...
      list.innerHTML += `
        <div data-id="${debt.id}" class="edit-card cursor-pointer bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card border border-red-50 dark:border-gray-700 flex flex-col gap-2 active:scale-[0.98]">
          ...
          <div class="flex justify-between items-center mt-1">
            <div class="text-[10px] text-gray-400"><p>EMI: ₹${Math.round(debt.emi).toLocaleString('en-IN')}</p><p>${progress}% Paid</p></div>
  ...
  list?.addEventListener('click', (e) => {
    ...
    if (document.getElementById('debtDate') && debt.date) document.getElementById('debtDate').value = debt.date.split('T')[0];
