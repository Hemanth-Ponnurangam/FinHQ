export function initUI() {
  ...
  // Track the pending closeAll timer so openSheet can cancel it.
  // Without this, if openSheet is called before the 300ms closeAll delay
  // fires, the timer re-adds 'hidden' to the sheet that was just opened,
  // causing the receipt (or any sheet) to instantly disappear — appearing
  // as if the tap did nothing ("freeze / nothing shows up").
  let closeAllTimer = null;

  function openSheet(sheetElement) {
    if (!sheetElement || !overlay) return;

    // Cancel any pending closeAll timer so it can't force-close this sheet.
    if (closeAllTimer !== null) {
