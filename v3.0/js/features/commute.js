...
export function initCommute(ui) {
  ...
  // ── Number Plate type switcher ───────────────────────────────
  const plateStyles = {
    private:    { bg: '#ffffff', text: '#000000', border: '#1f2937', strip: '#003399' },
    commercial: { bg: '#FFFF00', text: '#000000', border: '#1f2937', strip: '#003399' },
    ev:         { bg: '#00873D', text: '#ffffff', border: '#1f2937', strip: '#003399' },
  };
  let _plateType = 'private';

  function applyPlateStyle(type) {
    _plateType = type;
    const s  = plateStyles[type] || plateStyles.private;
    const pd = document.getElementById('plateDisplay');
    const pi = document.getElementById('fuelPlate');
    if (pd) { pd.style.background = s.bg; pd.style.borderColor = s.border; }
    if (pi) { pi.style.color = s.text; pi.style.background = 'transparent'; }
    // Update chips
    document.querySelectorAll('.plate-type-btn').forEach(btn => {
      const active = btn.dataset.platetype === type;
      btn.className = `plate-type-btn${active ? ' active-plate' : ''} text-[9px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-lg border-2 transition-all ${
        active ? '' : 'border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400'
      }`;
    });
  }

  document.querySelectorAll('.plate-type-btn').forEach(btn => {
    btn.addEventListener('click', () => applyPlateStyle(btn.dataset.platetype));
  });

  // Auto-detect type from fuelType field (EV → green plate)
  document.getElementById('fuelType')?.addEventListener('change', e => {
    if (e.target.value === 'ev' && _plateType !== 'ev') applyPlateStyle('ev');
    else if (e.target.value !== 'ev' && _plateType === 'ev') applyPlateStyle('private');
