...
// ── XSS helper ────────────────────────────────────────────────────────────
// Transaction titles, tags, and recurring names are free-text user input and
// must be escaped before innerHTML injection.
        ...
        html += `
          <div class="flex items-center justify-between py-2 border-b border-gray-50 dark:border-gray-700 last:border-0">
            <div class="flex-1 min-w-0 mr-3">
              <p class="text-sm font-medium dark:text-white truncate">${escHtml(t.title || 'Transaction')}</p>
              ${(t.tags || []).length ? `<p class="text-[10px] text-gray-400">${t.tags.map(escHtml).join(' · ')}</p>` : ''}
            </div>
      ...
      dueRecurring.forEach(r => {
        html += `
          <div class="flex items-center justify-between py-2 border-b border-gray-50 dark:border-gray-700 last:border-0 opacity-80">
            <div class="flex-1 min-w-0 mr-3">
              <p class="text-sm font-medium dark:text-white truncate">${escHtml(r.name)}</p>
              <p class="text-[10px] text-amber-500">🔄 Recurring · ${escHtml(r.type || 'Auto')}</p>
            </div>
    ...
    dayMaps.forEach(({ d, income, expense }, i) => {
      ...
      html += `
        <div class="flex flex-col items-center gap-1">
          ...
          ${hasRecurring ? `<div class="flex gap-0.5 flex-wrap justify-center" title="${recurringToday.map(r => escHtml(r.name)).join(', ')}">
